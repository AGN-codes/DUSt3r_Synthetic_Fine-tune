...
def load_crop_and_save(root, img, out_dir):
    ...
    #intrinsics_in, R_camin2world, t_camin2world = _load_pose(osp.join(root, 'cams', img + '_cam.txt'))
    color_image_in = cv2.cvtColor(cv2.imread(osp.join(root, 'image', img + # blended_images to image
                                  '.jpg'), cv2.IMREAD_COLOR), cv2.COLOR_BGR2RGB)
    depthmap_in = load_pfm_file(osp.join(root, 'depth_map', img + '.pfm')) # rendered_depth_maps to depth_map
